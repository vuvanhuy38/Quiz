...
function renderQuestions() {
    ...
    container.innerHTML = state.questions.map((q, index) => {
        ...
        const optionsHtml = (q.options || []).map(opt => `
            <div class="option-item ${isMulti ? "multi" : ""}"
        ...
        return `
            ...
            : ""}
                    ...
                    ${q.type
            ? `<span class="badge-type">${formatType(q.type)}</span>`
            : ""}
...
function refreshOptionUI(questionId, isMulti) {
    ...
    const opts =
        document.querySelectorAll(`[data-qid="${questionId}"]`);
...
function renderTimer() {
    ...
    const hours =
        Math.floor(state.secondsLeft / 3600);

    const minutes =
        Math.floor((state.secondsLeft % 3600) / 60);
...
function confirmSubmit() {
    ...
    // Đã làm hết
    const summary =
        `Bạn đã trả lời <strong>${answered}/${total}</strong> câu hỏi.`;
...
async function submitExam() {
    ...
    try {
        ...
        if (!res.ok) {
            ...
            throw new Error(
                body.message || `Lỗi khi nộp bài (${res.status})`
            );
        }
